...
def replace_chain():
    ...
    return jsonify(response), 200    
app.run(host = '0.0.0.0', port = 5001)#If you have the debugger disabled or trust the users on your network, you can make the server publicly available simply by adding --host=0.0.0.0 to the command line:
                                        #500 is port number
